...
# Rotation speed while a drone is in transit.
# 1.0 rotates during the whole turn.
# 4.0 finishes rotation during the first quarter of the turn.
TRANSIT_ROTATION_SPEED = 4.0
...
class ReplayWindow(arcade.Window):
    ...
    def on_update(self, delta_time: float) -> None:
        ...
        # Smoothstep produces softer movement at both ends.
        eased_progress = progress * progress * (
            3.0 - 2.0 * progress
        )

        # Rotation can finish earlier than the route movement.
        rotation_progress = min(
            progress * TRANSIT_ROTATION_SPEED,
            1.0,
        )
        eased_rotation_progress = (
            rotation_progress
            * rotation_progress
            * (3.0 - 2.0 * rotation_progress)
        )
        ...
        for drone_id, action in self.actions.items():
            ...
            sprite.angle = (
                start_angle
                + angle_delta * eased_rotation_progress
            )
